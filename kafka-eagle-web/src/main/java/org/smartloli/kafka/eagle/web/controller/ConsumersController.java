...
public class ConsumersController {
    ...
    private Logger LOG = LoggerFactory.getLogger(ConsumersController.class);
    /** Consumer viewer. */
    ...
    @RequestMapping(value = "/consumer/{group}/table/ajax", method = RequestMethod.GET)
        public void consumerTableListAjax(@PathVariable("group") String group, HttpServletResponse response, HttpServletRequest request) {
            ...
            String formatter = SystemConfigUtils.getProperty("kafka.eagle.offset.storage");
            JSONArray consumerDetails = JSON.parseArray(consumerService.getConsumerDetail(clusterAlias,formatter, group.replaceAll("==","/")));
            int offset = 0;
            JSONArray aaDatas = new JSONArray();
